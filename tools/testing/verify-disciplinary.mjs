...
async function main() {
  ...
  try {
    ...
    console.log('\n=== 1. Real UI workflow ===');
    {
      ...
      // underlying "Ready" label the component actually sets.
    ...
    for (const fx of fixtures) {
      ...
      const overflowReport = await page.evaluate(() => {
        const pages = Array.from(document.querySelectorAll('.docPdfExportRoot .docPdfPage'));
      ...
      await page.addStyleTag({ content: '.docPdfExportRoot { position: static !important; left: 0 !important; top: 0 !important; }' });
      const pngPath = path.join(outDir, `${fx.label}-page1.png`);
      await page.locator('.docPdfExportRoot .docPdfPage').first().screenshot({ path: pngPath }).catch(() => {});
