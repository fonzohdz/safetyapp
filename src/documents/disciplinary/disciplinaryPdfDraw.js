...
const SECTIONS = [
  ...
  [2, 'Earlier Warnings / Discussions', 'earlierWarnings'],
  [3, 'Company Policy States', 'companyPolicyStates'],
  [4, 'Employee Statement', 'employeeStatement'],
  [5, 'Corrective Action Required of Employee', 'correctiveActionRequired'],
...
export async function drawDisciplinaryPdf(model, onProgress) {
  ...
  doc.infoTable([
    ['Employee Name', model.employeeName, 'Date', fmtDate(model.noticeDate)],
    ['Supervisor', model.supervisor, 'Position', model.position],
  ], 0.19);

  doc.space(6);
  doc.grayBar('Warning Level');
  doc.checkboxGrid({
