...
Navigation is hand-rolled with three pieces of state in `App`: `tab` (top-level section: home/documents/drafts/templates/settings), `activeDoc` (`null | 'jsa-start' | 'jsa'`, which document workflow is open), and `jsaStep` (which step of the 6-step JSA wizard: `job` → `meeting` → `work` → `review` → `signatures` → `export`, defined in the `STEPS` array). The `review` step (content checklist + preview) deliberately sits *before* `signatures` (2026-08-19) — the crew's actual review happens verbally at the tailgate meeting before they sign, so the in-app checklist moment belongs there too, not after signing. `export` (labeled "Finish & Export" in the UI) is the lean, purely mechanical last step: Create Document, Save as Template, Document Options — no checklist of its own. Reaching `signatures`/`export` via a direct `StepNav` jump is blocked (`guardedJump` in `JsaWorkflow`) unless `job`/`meeting`/`work` are all complete, so a JSA can't be signed with no content on it.
...
- **Single draft slot.** Only one JSA draft exists at a time; starting blank or loading a template replaces it. This used to happen with no unsaved-changes guard, but `requestStartBlank()`/`requestLoadTemplate()` (main.jsx) now check `hasMeaningfulJsaContent()` and route through the same `ConfirmReplaceDialog` `clearDraft()` uses before actually discarding anything (verified still true 2026-08-19). Keep this guard in mind if asked to add new entry points that discard the current draft — reuse `hasMeaningfulJsaContent`/`ConfirmReplaceDialog` rather than adding a parallel confirm.
